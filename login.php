...
echo "<script>alert('No ha sido registrado, ocurrio un error');</script>";
}
}
// Code for User login
if(isset($_POST['login']))
{
   ...
   $password=md5($_POST['password']);
	...
	{
		$query=mysqli_query($con,"SELECT * FROM admin WHERE username='$email' and password='$password'");
		$num=mysqli_fetch_array($query);
		if($num>0)
		{
			$extra="admin/change-password.php";//
			...
			$uri  = rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
			header("location:http://$host$uri/$extra");
			$_SESSION['errmsg']="Email id o Password incorrectos";
			exit();
		}
	}
}
...
<html lang="es">
	<head>
	    ...
	    <title>Tienda Virtual RosiMarquet | Signi-in | Signup</title>
...
<!-- ============================================== TOP MENU : END ============================================== -->
<?php include('includesPrincipal/main-header.php');?>
	<!-- ============================================== NAVBAR ============================================== -->
<?php include('includesPrincipal/menu-bar.php');?>
<!-- ============================================== NAVBAR : END ============================================== -->
	...
	<div class="container">
		<div class="breadcrumb-inner">
			<ul class="list-inline list-unstyled">
				...
				<li class='active'>Autentificatión</li>
			</ul>
...
?>
		...
		<div class="form-group">
		    ...
		    <input type="email" name="email" class="form-control unicase-form-control text-input" id="exampleInputEmail1" pattern="^[^@]+@[^@]+\.[a-zA-Z]{2,}$">
		</div>
	  	<div class="form-group">
...
<div class="form-group">
	    	...
	    	<input type="text" class="form-control unicase-form-control text-input" id="fullname" name="fullname" pattern="^[A-Za-z]+((\s)?([A-Za-z])+)*$" required="required">
		...
		<div class="form-group">
	    	...
	    	<input type="email" class="form-control unicase-form-control text-input" id="email" name="emailid" pattern="^[^@]+@[^@]+\.[a-zA-Z]{2,}$" required="required">
	    	       <span id="user-availability-status1" style="font-size:12px;"></span>
	  	</div>

<div class="form-group">
	    	<label class="info-title" for="contactno">elular. <span>*</span></label>
	    	<input type="text" class="form-control unicase-form-control text-input" id="contactno" name="contactno" maxlength="10" pattern="[0-9]{8}" required="required">
	  	</div>
...
<?php include('includesPrincipal/footer.php');?>
